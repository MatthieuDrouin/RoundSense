...
import {MAP_DATA, onRadar, radarUrl, worldToRadar} from './mapData';
import './styles.css';
...
function TacticalMap({state}){
  ...
  const bomb=state.bomb_x!=null&&state.bomb_y!=null?worldToRadar(state.map_name,state.bomb_x,state.bomb_y):null;
  ...
  return <div className="radarWrap">
    <div className="radar">
      {bg&&<img src={bg} alt={state.map_name} onError={e=>{e.currentTarget.style.display='none'}}/>}
      {meta.a&&<span className="site siteA" style={{left:`${meta.a[0]*100}%`,top:`${meta.a[1]*100}%`}}>A</span>}
      {meta.b&&<span className="site siteB" style={{left:`${meta.b[0]*100}%`,top:`${meta.b[1]*100}%`}}>B</span>}
      {positioned.map(p=>{
        const pt=worldToRadar(state.map_name,p.x,p.y);
    ...
    <div className="legend"><span><i className="ctLegend"></i>CT</span><span><i className="tLegend"></i>T</span><span><i className="bombLegend"></i>Bomb</span><small>{positioned.length} positioned players</small></div>
  </div>
}
...
function TacticalIntel({tactical}){
  ...
  return <>
    ...
    <div className="intelRow"><span>Positioning model adjustment</span><b className={adj<0?'tAdv':adj>0?'ctAdv':''}>{signed(adj)} log-odds</b></div>
...
function App(){
  ...
  return <main>
    ...
    <section className="mapGrid">
      <article className="mapCard"><div className="cardTitle"><h2>Live Tactical Map</h2><small>Player positions influence the prediction when full-team GSI is available.</small></div><TacticalMap state={state}/></article>
      <article><h2>Positioning Intelligence</h2><TacticalIntel tactical={state.tactical}/><div className="spatialStats">
    ...
    <section className="grid">
      ...
      <article className="players"><h2>Top Players</h2>{topPlayers.map((p,i)=><div className="player" key={p.steam_id}><span>#{i+1} {p.name}<em>{p.team}</em></span><span>{p.kills}/{p.deaths} · {Number(p.impact||0).toFixed(2)}</span></div>)}</article>
